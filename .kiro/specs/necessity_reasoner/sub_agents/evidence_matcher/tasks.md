# Tasks — `evidence_matcher`

> Generated from Authrex `AGENT_MANIFEST` at 2026-05-02T13:05:55.346095+00:00.

## Implementation tasks

- [ ] Define `EvidenceMatcherInput` (input) and `EvidenceMatch` (output) in the package's `schemas.py`.
- [ ] Subclass `Agent[InputSchema, OutputSchema]`, declaring required ClassVars (`name`, `parent`, `role`, `description`).
- [ ] Place implementation in `backend/app/agents/necessity_reasoner/sub_agents/evidence_matcher.py`.

- [ ] Author `system_prompt` in `backend/app/prompts/necessity_reasoner/sub_agents/evidence_matcher.txt` (or `evidence_matcher.txt` for orchestrators).
...
- [ ] Add a contract test under `backend/tests/agents/test_evidence_matcher.py`.
