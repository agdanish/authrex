# Tasks — `appeals_drafter`

> Generated from Authrex `AGENT_MANIFEST` at 2026-05-02T13:05:55.309689+00:00.

## Implementation tasks

- [ ] Define `AppealsDrafterInput` (input) and `AppealsDrafterOutput` (output) in the package's `schemas.py`.
- [ ] Subclass `Agent[InputSchema, OutputSchema]`, declaring required ClassVars (`name`, `parent`, `role`, `description`).
- [ ] Place implementation in `backend/app/agents/appeals_drafter/orchestrator.py`.

- [ ] Implement `_execute_deterministic(input, ctx) -> output` in `backend/app/agents/appeals_drafter/orchestrator.py`.
- [ ] Set `primary_model = None` so the framework knows to skip the LLM path.


## Test tasks

- [ ] Add a contract test under `backend/tests/agents/test_appeals_drafter.py`.
