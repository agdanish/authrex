# Tasks — `action_step_writer`

> Generated from Authrex `AGENT_MANIFEST` at 2026-05-02T13:05:55.355041+00:00.

## Implementation tasks

- [ ] Define `ActionStepWriterInput` (input) and `ActionStepWriterOutput` (output) in the package's `schemas.py`.
- [ ] Subclass `Agent[InputSchema, OutputSchema]`, declaring required ClassVars (`name`, `parent`, `role`, `description`).
- [ ] Place implementation in `backend/app/agents/patient_communicator/sub_agents/action_step_writer.py`.

- [ ] Author `system_prompt` in `backend/app/prompts/patient_communicator/sub_agents/action_step_writer.txt` (or `action_step_writer.txt` for orchestrators).
...
- [ ] Add a contract test under `backend/tests/agents/test_action_step_writer.py`.
