...
> Generated from Authrex `AGENT_MANIFEST` at 2026-05-02T13:05:55.351348+00:00.

## Implementation tasks

- [ ] Define `EmpathyLayerInput` (input) and `EmpathyLayerOutput` (output) in the package's `schemas.py`.
- [ ] Subclass `Agent[InputSchema, OutputSchema]`, declaring required ClassVars (`name`, `parent`, `role`, `description`).
- [ ] Place implementation in `backend/app/agents/patient_communicator/sub_agents/empathy_layer.py`.

- [ ] Author `system_prompt` in `backend/app/prompts/patient_communicator/sub_agents/empathy_layer.txt` (or `empathy_layer.txt` for orchestrators).
...
- [ ] Add a contract test under `backend/tests/agents/test_empathy_layer.py`.
