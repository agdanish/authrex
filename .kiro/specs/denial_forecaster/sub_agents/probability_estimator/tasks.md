# Tasks — `probability_estimator`

> Generated from Authrex `AGENT_MANIFEST` at 2026-05-02T13:05:55.335943+00:00.

## Implementation tasks

- [ ] Define `ProbabilityEstimatorInput` (input) and `ProbabilityEstimatorOutput` (output) in the package's `schemas.py`.
- [ ] Subclass `Agent[InputSchema, OutputSchema]`, declaring required ClassVars (`name`, `parent`, `role`, `description`).
- [ ] Place implementation in `backend/app/agents/denial_forecaster/sub_agents/probability_estimator.py`.

- [ ] Author `system_prompt` in `backend/app/prompts/denial_forecaster/sub_agents/probability_estimator.txt` (or `probability_estimator.txt` for orchestrators).
...
- [ ] Add a contract test under `backend/tests/agents/test_probability_estimator.py`.
