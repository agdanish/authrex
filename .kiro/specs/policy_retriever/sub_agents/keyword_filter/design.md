# Design — `keyword_filter`

> Generated from Authrex `AGENT_MANIFEST` at 2026-05-02T13:05:55.358305+00:00.
...
## Input schema (`KeywordFilterInput`)
...
```json
{
  ...
  "title": "KeywordFilterInput",
  "type": "object"
}
```

## Output schema (`KeywordFilterOutput`)
...
1. Validate input against `KeywordFilterInput` (Pydantic).
...
6. Parse output into `KeywordFilterOutput` with retry-on-failure (Haiku→Sonnet escalation).
...
- `authrex_agent_invocations_total{agent="keyword_filter",status=...}`
